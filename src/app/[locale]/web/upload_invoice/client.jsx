...
export default function ClientInvoice() {
	...
	const {files, selectedFile, addFilesToSlot, removeFile, update} = useUploadFileState(
		(state) => state,
	)
	...
	const currentStep     = files.length === 0 ? 1 : 2
	const hasNewFiles     = files.some((f) => f.isNew)
	const invoiceDisabled = files.some((f) => f.slotKey === 'invoice')
	...
	// 	console.log('Saving:', {item, files})
	// }


const handleSave = () => {
    // tú lees el state aquí, no el schema
    const {item, files} = useUploadFileState.getState()
		...
		<div className="flex flex-col gap-4">
			<div className="flex flex-col gap-2">
				...
				<div className="grid grid-cols-2 gap-2">
					...
					/>
					<DropzoneSlotMulti slotKey="other" onDrop={handleDrop} />
				</div>
			</div>
		...
		<div className="flex h-full w-full flex-col overflow-hidden bg-slate-100 dark:bg-slate-900">
			...
			<header className="flex items-center justify-between border-b border-slate-200 bg-white px-4 py-3 shadow-sm dark:border-slate-700 dark:bg-slate-800">
				...
				<div className="flex items-center gap-2">
					<button className="rounded-lg border border-slate-200 px-3 py-1.5 text-xs text-slate-500 transition-all hover:bg-slate-50 md:px-4 md:text-sm dark:border-slate-600 dark:text-slate-400 dark:hover:bg-slate-700">
						Cancel
					</button>
					<button
						onClick={handleSave}
			...
			<div className="border-b border-slate-200 px-4 py-2 dark:border-slate-700 dark:bg-slate-800">
				<StepperBar currentStep={currentStep} />
			</div>

			<div className="flex border-b border-slate-200 md:hidden dark:border-slate-700 dark:bg-slate-800">
				<button
			...
			<div className="flex flex-1 overflow-hidden">
				...
				<div className="hidden flex-1 overflow-hidden md:flex">
					<aside className="w-95 shrink-0 overflow-y-auto border-r border-slate-200 bg-white p-4 dark:border-slate-700 dark:bg-slate-800">
						{LeftPanel}
					</aside>
